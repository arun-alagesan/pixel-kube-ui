...
const addModel = (modelResponse:any) => {
     if(modelResponse!=null && modelResponse.connectorResponse!=null && modelResponse.connectorResponse.length)
      {
...
export default function AddButton({onClose, calenderValue,modelResponse}: any , props:any) {
    const SelectedButton: any = ButtonSelector[calenderValue ? calenderValue : '70']
    return modelResponse!=null && modelResponse.connectorResponse!=null && modelResponse.connectorResponse.length>0?(<button className="btn btn-primary w-full" onClick={()=>addModel(modelResponse)}>Add Connector</button>):(
